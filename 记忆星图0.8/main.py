...
import pickle
pygame.init()
def ls2imga(index,dic):
    return pygame.image.fromstring(dic[index][0],dic[index][1],"RGBA").convert_alpha()
def ls2img(index,dic):
    return pygame.image.fromstring(dic[index][0],dic[index][1],"RGB").convert_alpha()
def init():
    global imgls,angls,sidls,bgm,star
    with open("main.pdb","rb") as file:
        tmp = pickle.load(file)
    star4 = ls2imga("4.png",tmp)
    star5 = ls2imga("5.png",tmp)
    star6 = ls2imga("6.png",tmp)
    star = [star4,star5,star6]
    starlock = ls2imga("lock.png",tmp)
        ...
        for j in temp:
            if j[2] == 1:
                ...
                if f"星座/{name}/{k}.jpg" in tmp:
                    ...
                    imgls[name].append([None,j[0],j[1],starlock,0])
                    angls[name].append(0)
...
logo2 = ls2imga("logo2.png",temp).convert_alpha()
logo = ls2imga("logo.png",temp).convert_alpha()
    ...
    for i in imgls:
        ...
        for j in range(len(imgls[i])):
            ...
            if imgls[i][j][1] > 0 and imgls[i][j][1] < screen.get_size()[0]:
                in_screen = True
            rect.center = imgls[i][j][1:3]
            if rect.collidepoint(mouse) and imgls[i][j][0] != None:
